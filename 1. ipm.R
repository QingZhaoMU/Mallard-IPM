...
path <- 'c:/Zhao/1. Mallard/'

#==============
# read in data
#==============
# bpop
bpop <- read.csv(paste(c(path, 'data/bpop.csv'), collapse=''))
bpop_se <- read.csv(paste(c(path, 'data/bpop_se.csv'), collapse=''))
...
# climate
temp <- read.csv(paste(c(path, 'data/temp.csv'), collapse=''))
prec <- read.csv(paste(c(path, 'data/prec.csv'), collapse=''))
...
prec <- log(prec)
prec <- (prec - mean(prec)) / sd(prec)

# band-recovery
load(paste(c(path, 'data/marr.RData'), collapse=''))
...
data {
  ...
  matrix[narea, nyear] prec; # winter precipitation
...
parameters {
  ...
  real D0_beta_prec; # precipitation effect on initial population density
  ...
  real gamma_beta_prec; # precipitation effect on reproduction
  real gamma_beta_ddp_prec; # interaction of density and precipitation on reproduction
  ...
  real omega_beta_prec; # precipitation effect on survival
  real omega_beta_ddp_prec; # interaction of density and precipitation on survival
  ...
  for (i in 1:narea) {
    for (t in 1:(nyear-1)) {
      log_gamma_mu[i,t] <- 
        ...
        gamma_beta_prec * prec[i,t] + 
        gamma_beta_ddp_prec * (log(D[i,t]) - mean(log(D[]))) / sd(log(D[])) * prec[i,t];
  ...
  for (i in 1:narea) {
    for (t in 1:nyear) {
      logit_omega_mu[i,t] <- 
        ...
        omega_beta_prec * prec[i,t] + 
        omega_beta_ddp_prec * (log(D[i,t]) - mean(log(D[]))) / sd(log(D[])) * prec[i,t];
...
model {
  ...
  gamma_beta_prec ~ normal(0, 100); 
  gamma_beta_ddp_prec ~ normal(0, 100); 
  ...
  omega_beta_prec ~ normal(0, 100); 
  omega_beta_ddp_prec ~ normal(0, 100); 
...
# setup for STAN
#================
data <- list(narea=narea, nyear=nyear, temp=temp, prec=prec, marr=marr, 
             bpop_obs=bpop_obs, bpop_obs_sigma=bpop_obs_sigma)

init_fun <- function() {
              list(
                D0_alpha=0, D0_beta_temp=0, D0_beta_prec=0, D0_sigma=1, 
                gamma_alpha_mu=rep(0,nbcr), gamma_alpha_sigma=rep(1,nbcr), 
                gamma_alpha=rep(0,narea), gamma_beta_ddp=0, 
                gamma_beta_temp=0, gamma_beta_ddp_temp=0, 
                gamma_beta_prec=0, gamma_beta_ddp_prec=0, 
                gamma_sigma=1, 
                log_gamma=matrix(0, narea, nyear-1), 
                omega_alpha_mu=rep(0,nbcr), omega_alpha_sigma=rep(1,nbcr), 
                omega_alpha=rep(0,narea), omega_beta_ddp=0, 
                omega_beta_temp=0, omega_beta_ddp_temp=0, 
                omega_beta_prec=0, omega_beta_ddp_prec=0, 
                omega_sigma=1, 
...
fit <- stan(file=paste(c(path, 'ipm.stan'), collapse=''), 
            ...
            par=c('D0_alpha', 'D0_beta_temp', 'D0_beta_prec', 'D0_sigma', 
                  'gamma_alpha_mu', 'gamma_alpha_sigma', 'gamma_alpha', 
                  'gamma_beta_ddp', 
                  'gamma_beta_temp', 'gamma_beta_ddp_temp', 
                  'gamma_beta_prec', 'gamma_beta_ddp_prec',  
                  'gamma_sigma', 
                  'omega_alpha_mu', 'omega_alpha_sigma', 'omega_alpha', 
                  'omega_beta_ddp', 
                  'omega_beta_temp', 'omega_beta_ddp_temp',  
                  'omega_beta_prec', 'omega_beta_ddp_prec',  
                  'omega_sigma', 
                  'logit_rec_mu', 'logit_rec_sigma', 
                  'rec', 
                  'gamma', 'omega', 
                  'D', 'R', 'S'), 
            iter=2500, warmup=500, thin=1, chains=5)
...
#===============
print(fit, digits=3, 
      c('D0_alpha', 'D0_beta_temp', 'D0_beta_prec', 'D0_sigma', 
        'gamma_alpha_mu', 'gamma_alpha_sigma', 'gamma_alpha', 
        'gamma_beta_ddp', 
        'gamma_beta_temp', 'gamma_beta_ddp_temp',  
        'gamma_beta_prec', 'gamma_beta_ddp_prec',  
